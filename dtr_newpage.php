...
<body>
    <div class="container">
        <h3>DTR Records</h3>
        <form action="record_time_newpage.php" method="post" id="recordForm">
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {
                        ...
                        // Check if the employee has a shift today (Manila timezone)
                        var today = moment().tz("Asia/Manila").format('dddd');
                        if (!result.shifts[today]) {
                            // Disable all buttons
                            disableButtons();
                            // Show alert message
                            Swal.fire({
                                icon: 'info',
                                title: 'Info',
                                text: "You don't have a shift today."
                            });
                            return;
                        }
