...
<div class="container mt-5">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h2 class="text-center">Edit Tax</h2>
        </div>
        <div class="card-body">
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $employee_id; ?>">
                ...
                <button type="submit" class="btn btn-primary btn-block" name="update_tax" onclick="return validateTax()">Update Tax</button>
            </form>
        </div>
    </div>
</div>
...
<script>
    ...
    function validateTax() {
        ...
        if (taxValue.trim() === "") {
            showAlert("Tax value cannot be empty.", "error");
