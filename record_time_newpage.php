...
// Check if employee_number is provided in the form
if(isset($_POST['employeeNumber'])) {
    // Retrieve the employee number from the form
    ...
    $stmt_find_employee->bind_param("s", $employee_number);
    ...
    if ($result_find_employee->num_rows > 0) {
        ...
        // If employee is not found, show an error message and exit
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["time_in"])) {
    ...
    if ($result_check->num_rows > 0) {
        echo "<script>alert('You have already timed in today!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>alert('Time In Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["time_out"])) {
    ...
    if ($result_check->num_rows > 0) {
        echo "<script>alert('You have already timed out today!'); window.location.href = 'dtr_newpage.php'; </script>";
    } else {
        ...
        $stmt_check_in = $connection->prepare($sql_check_in);
        $stmt_check_in->bind_param("ss", $employee_id, $currentDate);
        ...
        if ($result_check_in->num_rows == 0) {
            echo "<script>alert('You must time in first before timing out!'); window.location.href = 'dtr_newpage.php'; </script>";
        } else {
            // Get the current time
            $currentTime = date("H:i:s");
            ...
            if ($stmt->execute()) {
                // Redirect to dtrRevisedManager.php with the employee_id parameter
                echo "<script>alert('Time Out Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
            } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["break_out"])) {
    ...
    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Break Out Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["break_in"])) {
    ...
    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Break In Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["broken_time_in"])) {
    // Get the current date
    $currentDate = date("Y-m-d");
    // Get the current date and time
    $currentDateTime = date("Y-m-d H:i:s");
    ...
    // Insert the current date and time as break in time into the database
    $sql = "UPDATE dtrrevised SET broken_time_in = ? WHERE employee_id = ? AND date = ?";
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("sss", $currentDateTime, $employee_id, $currentDate);

    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Broken Time In Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
        echo "Error recording Broken Time In: " . $connection->error;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["broken_break_out"])) {
    // Get the current date
    $currentDate = date("Y-m-d");
    // Get the current date and time
    $currentDateTime = date("Y-m-d H:i:s");
    ...
    // Insert the current date and time as break in time into the database
    $sql = "UPDATE dtrrevised SET broken_break_out = ? WHERE employee_id = ? AND date = ?";
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("sss", $currentDateTime, $employee_id, $currentDate);

    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Broken Break Out Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
        echo "Error recording Break In: " . $connection->error;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["broken_break_in"])) {
    // Get the current date
    $currentDate = date("Y-m-d");
    // Get the current date and time
    $currentDateTime = date("Y-m-d H:i:s");
    ...
    // Insert the current date and time as break in time into the database
    $sql = "UPDATE dtrrevised SET broken_break_in = ? WHERE employee_id = ? AND date = ?";
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("sss", $currentDateTime, $employee_id, $currentDate);

    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Broken  Break In Recorded Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
        echo "Error recording Break In: " . $connection->error;
    }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["broken_time_out"])) {
    // Get the current date
    $currentDate = date("Y-m-d");
    // Get the current date and time
    $currentDateTime = date("Y-m-d H:i:s");
    ...
    // Insert the current date and time as break in time into the database
    $sql = "UPDATE dtrrevised SET broken_time_out = ? WHERE employee_id = ? AND date = ?";
    $stmt = $connection->prepare($sql);
    $stmt->bind_param("sss", $currentDateTime, $employee_id, $currentDate);

    if ($stmt->execute()) {
        // Redirect to dtrRevisedManager.php with the employee_id parameter
        echo "<script>alert('Broken Time Out Successfully!'); window.location.href = 'dtr_newpage.php';</script>";
    } else {
        echo "Error recording Break In: " . $connection->error;
    }
